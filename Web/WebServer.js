...
module.exports = (bot, db, auth, config, winston) => {
  ...
  if (config.cert && config.privKey && config.httpsPort) {
    ...
    const privKey = fs.readFileSync(config.privKey, "utf8", function (err) {
      if (err) winston.error(err);
    });
    const cert = fs.readFileSync(config.cert, "utf8", function (err) {
      if (err) winston.error(err);
    });
    const credentials = {
      key: privKey,
    ...
    if (req.isAuthenticated()) {
      ...
      db.users.findOne({
        ...
        if (userDocument) {
          ...
          let serverIDs = servers.map(server => {
            return server.id
          });

          let matchCriteria = {};
          if (req.query.server != "All") {
            if (serverIDs.find(id => {return req.query.server === id})) {
  ...
  const saveAdminConsoleOptions = (consolemember, svr, serverDocument, req, res, override) => {
    serverDocument.save(err => {
      ...
      if (err) {
        winston.error(`Failed to update settings at ${req.path}`, {
          svrid: svr.id,
          usrid: consolemember.id
        }, err);
      }
      if (override) {
        res.sendStatus(200);
    ...
    checkAuth(req, res, () => {
      if (req.body.message) {
        bot.guilds.forEach(svr => {
          ...
          const tmpChannel = this.channels.filter((c) => c.type === 0 && c.permissionsOf(this.shard.client.user.id).has("readMessages")).sort((a, b) => a.position - b.position)[0];
