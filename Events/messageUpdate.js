		...
		db.servers.findOne({_id: msg.channel.guild.id}, (err, serverDocument) => {
			if(!err && serverDocument) {
				...
				if(oldmsgdata.cleanContent && msg.cleanContent && oldmsgdata.cleanContent!=msg.cleanContent) {
					...
					if(serverDocument.config.moderation.isEnabled && serverDocument.config.moderation.status_messages.message_edited_message.isEnabled && serverDocument.config.moderation.status_messages.message_edited_message.enabled_channel_ids.indexOf(msg.channel.id)>-1) {
						...
						if(serverDocument.config.moderation.status_messages.message_edited_message.type=="single" && serverDocument.config.moderation.status_messages.message_edited_message.channel_id) {
							const ch = msg.channel.guild.channels.get(serverDocument.config.moderation.status_messages.message_edited_message.channel_id);
							if(ch) {
						...
						// Send message in same channel
							...
							const channelDocument = serverDocument.channels.id(msg.channel.id);
                ...
                winston.error("Failed to find server data for messageUpdate", {
