...
module.exports = (bot, db, config, winston, userDocument, serverDocument, channelDocument, memberDocument, msg, suffix) => {
    ...
    if (suffix) {
        if (suffix.toLowerCase() == "add") {
                ...
                db.events.create({
                    ...
                    if (err) {
                        ...
                        QueryHelper.findServerEvents(db, serverDocument._id).then((eventDocuments) => {
                            let viewer = new EventViewer(db, serverDocument, eventDocuments, userDocument, msg.member, page_size);
            ...
            QueryHelper.findServerEvents(db, serverDocument._id).then((eventDocuments) => {
                viewer = new EventViewer(db, serverDocument, eventDocuments, userDocument, msg.member, page_size);
