...
module.exports = (bot, db, config, winston, userDocument, serverDocument, channelDocument, memberDocument, msg, suffix, commandData) => {
    ...
    if (roomDocument) {
                ...
                if (config.yes_strings.indexOf(message.content.toLowerCase().trim()) > -1) {
                    msg.channel.delete().then(() => {
                        ...
                        serverDocument.save(err => {
                            if (err) {
                                winston.error("Failed to save server data for room deletion", { svrid: msg.guild.id }, err);
                            }
                        });
                    }).catch(err => {
                        winston.error(`Failed to delete room '${msg.channel.name}' on server '${msg.guild.name}'`, { svrid: msg.guild.id, chid: msg.channel.id }, err);
                                ...
                                msg.channel.createMessage("Please enter the names of the members you'd like to add, separated by `|`:").then(() => {
                                    bot.awaitMessage(msg.channel.id, msg.author.id, message => {
                                        message.content.split("|").forEach(membername => {
                                            const member = bot.memberSearch(membername, msg.guild);
                                            if (member) {
                                                msg.channel.editPermission(member.id, 3072, null, "member").then().catch(err => {
                                                    if (err) {
                                                        winston.error(`Failed to add member '${member.user.username}' to room '${msg.channel.name}' on server '${msg.guild.name}'`, { svrid: msg.guild.id, chid: msg.channel.id, usrid: member.id }, err);
                                                    }
                                                });
                                            }
                                        });
        ...
        msg.guild.createChannel(`G4M3R-room-${Date.now()}`, type).then(ch => {
            ...
            if (ch.type == 0) {
                ...
                ch.createMessage(`**First!** 🐬 Use \`${bot.getCommandPrefix(msg.guild, serverDocument)}${commandData.name}\` to delete this room or add members.`);
            }

            serverDocument.config.room_data.push({ _id: ch.id });
            serverDocument.save(err => {
                if (err) {
                    winston.error("Failed to save server data for room creation", { svrid: msg.guild.id }, err);
                }

                ch.editPermission(msg.guild.id, null, permissionConstant, "role").then(() => {
                    members.forEach(member => {
                        ch.editPermission(member.id, permissionConstant, null, "member").then().catch(err => {
                            winston.error(`Failed to add member '${member.user.username}' to room '${ch.name}' on server '${msg.guild.name}'`, { svrid: msg.guild.id, chid: ch.id, usrid: member.id }, err);
                        });
                    });
                }).catch(err => {
                    winston.error(`Failed to edit @everyone permissions for room '${ch.name}' on server '${msg.guild.name}'`, { svrid: msg.guild.id, chid: ch.id }, err);
                });
            });
        }).catch(err => {
            winston.error(`Failed to create ${suffix.split(" ")[0].toLowerCase()} room on server '${msg.guild.name}'`, { svrid: msg.guild.id }, err);
        ...
        msg.channel.createMessage(`${msg.author.mention} I didn't get that. Please use the syntax \`${bot.getCommandPrefix(msg.guild, serverDocument)}${commandData.name} ${commandData.usage}\` to create a room!`);
    }
};