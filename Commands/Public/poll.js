...
module.exports = (bot, db, config, winston, userDocument, serverDocument, channelDocument, memberDocument, msg, suffix, commandData) => {
	if(channelDocument.poll.isOngoing) {
		if(suffix) {
			...
			if(voteDocument) {
				msg.channel.createMessage(`You've already voted in this poll. PM me \`${commandData.name} ${msg.channel.guild.name}|#${msg.channel.name}\` to erase your vote.`);
			} else {
				let vote;
				if(isNaN(suffix)) {
		...
		msg.channel.createMessage(`There is no ongoing poll in this channel. 🛡 PM me \`${commandData.name} ${msg.channel.guild.name}|#${msg.channel.name}\` to start one.`);
	}
};