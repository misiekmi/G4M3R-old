...
module.exports = (bot, db, config, winston, userDocument, serverDocument, channelDocument, memberDocument, msg, suffix, commandData) => {
        if (suffix) {
            ...
            if (suffix.indexOf("|") > -1) {
                                    ...
                                    if (PermissionConstants.hasOwnProperty(perm)) {
                                        const doOverwrite = (allow, deny) => {
                                                msg.channel.editPermission(target.id, allow, deny, type).then(() => {
                                                            msg.channel.createMessage(`Done. **${type=="role" ? target.name : (`@${bot.getName(msg.channel.guild, serverDocument, target)}`)}** now ${allow==null ? "doesn't have" : "has"} the \`${perm}\` permission in #${msg.channel.name}`);
								}).catch(err => {
				...
				winston.warn(`Invalid parameters '${suffix}' provided for ${commandData.name} command`, {svrid: msg.channel.guild.id, chid: msg.channel.id, usrid: msg.author.id});
				msg.channel.createMessage(`${msg.author.mention} Invalid syntax - use \`${bot.getCommandPrefix(msg.channel.guild, serverDocument)}${commandData.name} <user or role>|<permission name>\``);
			}
		} else {
