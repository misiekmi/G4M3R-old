module.exports = (bot, db, config, winston, userDocument, serverDocument, channelDocument, memberDocument, msg, suffix, commandData) => {
	if(suffix) {
				...
				if(args.length==2 && args[0].trim()) {
					...
					if(member) {
						...
						bot.editGuildMember(msg.channel.guild.id, member.id, {nick: args[1].trim()}).then(() => {
							msg.channel.createMessage(`**@${bot.getName(msg.channel.guild, serverDocument, member, true)}** now has the nickname \`${member.nick}\``);
						}).catch(err => {
					...
					msg.channel.createMessage(`${msg.author.mention} huh? use \`${bot.getCommandPrefix(msg.channel.guild, serverDocument)}${commandData.name} <user>|<name>\` to change someone's nickname`);
				}
			} else {
		...
		if(msg.member.nick) {
			...
			msg.channel.createMessage(`You don't have a nick on this server. Use \`${bot.getCommandPrefix(msg.channel.guild, serverDocument)}${commandData.name} <name>\` to set one`);
		}
	}
};
